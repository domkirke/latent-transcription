...
parser.add_argument('-s', "--latent_sample", type=int, default=0, help="sampling")
...
parser.add_argument('--n_steps', type=int, default = 20000, help="number of steps for various generations")
...
parser.add_argument("--sequence", type=int, default=None, nargs="*")
parser.add_argument("--trajectory_types", type=str, default=["line"], nargs="*")
...
def chord2onehot(chords, label_params, mode='separated_octave'):
...
def midi2metadata(file_path, label_params, intervals = 0.02, keep_silences=False, keep_multipitch=False):    
    ...
    for i, off in enumerate(intervals):
        ...
        if idx < len(orderedChordList):
            metadatas.append(chord2onehot(orderedChordList[idx], label_params))
...
def midi2audio(model_path, midi_paths, device='cpu', out=None):
    def midi2audio_single(models, model_dicts, midi_path):
        ...
        metadata, silence_mask = midi2metadata(midi_path, models[1].pinput, keep_silences=True, keep_multipitch=True)
        ...
        resynthesize(signal_out, upsampleFactor=4, out=args.output+'/%s.wav'%filename) 
...
def resynthesize(x_in, grain_len = 2.0, upsampleFactor = 10, out=None, preprocessing=None, threshold = 1e-4, normalize=True, iterations=30):
    ...
    for i, chunk in enumerate(chunks):
        chunk_resynth = audio.regenerateAudioNSGT(chunk.T, 30, 11000, 48, iterations = iterations, targetLen = target_len, testSize=True)
...
def audio2model(model_path, audio_paths, out=args.output, sample=False):
    def audio2model_single(model, model_dicts, audio_path):
        ...
        # output signal
        signal_out = preprocessing.invert(signal_out)
        resynthesize(signal_out, upsampleFactor=1, out=args.output+'/%s.wav'%filename) 
...
def path2audio(model, current_z, n_interp=1, order_interp = 1, out=None, preprocessing=None, graphs=True):
    # make interpolation
    if order_interp == 0:
    ...
    model.eval()
    vae_out = model.decode( model.format_input_data(z_interp), sample=False )
    ...
    if not preprocessing is None:
        signal_out = preprocessing.invert(signal_out)
    resynthesize(signal_out, upsampleFactor=1, out=out+'.wav')      



def sequence2audio(model_path, sequence, n_interp=1, order_interp = 1, out=None,  preprocessing=None, device='cpu', graphs=True):
    # load model
    ...
    path2audio(models[0], current_z, n_interp=n_interp, order_interp=order_interp, preprocessing=preprocessing, graphs=graphs)
...
def trajectory2audio(model_path, traj_types, n_trajectories=1, n_steps=1000, order_interp = 1, out=None,  preprocessing=None, device='cpu', graphs=True):
    # load model
    models, model_dicts = load_model(model_path, args.cuda)
    ...
    for traj_type in traj_types:
        ...
        for i,t in enumerate(trajectories):
            path2audio(models[0], t, n_interp=1, preprocessing=preprocessing, out=out+'/trajectories/%s_%s'%(traj_type,  i))
...
if __name__ == "__main__":
    ...
    if args.mode == 'info':
        ...
        for model_path in args.models:
            ...
            midi2audio(model_path, args.input, out=args.output, device=args.cuda)
        ...
        for model_path in args.models:
            print('MODEL : %s'%model_path)
            audio2model(model_path, args.input, out=args.output)        
        ...
        for model_path in args.models:
            print('MODEL : %s'%model_path)
            sequence2audio(model_path, args.input, n_interp=args.n_steps, order_interp=0, out=args.output, device=args.cuda)
        ...
        for model_path in args.models:
            print('MODEL : %s'%model_path)
            sequence2audio(model_path, args.input, n_interp=args.n_steps, order_interp=args.interp_order, out=args.output, device=args.cuda)
        ...
        for model_path in args.models:
            ...
            trajectory2audio(model_path, args.trajectory_types, n_trajectories = args.n_generations, n_steps = args.n_steps, out=args.output, device=args.cuda, order_interp=args.interp_order)
